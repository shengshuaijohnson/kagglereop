...
def write_to_file(item, output_file='output.txt'):  # 为了便于数据库储存，使用json格式。如果要保存到csv，需要使用pandas的write_to_csv
...
def main():
    ...
    for bill_number in zto_numbers:
        if check_if_crawled(bill_number, 'success.txt') or check_if_crawled(bill_number, 'failed.txt'):
            continue
        # 起初只检查success.txt，后来因为多次断网重启并且有些单号是必定失败，效率起见改成两个都检查，爬完可以改回来
        print count
        count += 1
        try:
            if process_single_bill(bill_number):            # 之前没if导致多了4个连接error数据的差别，爬完以后再检查修正一下
